...
{*                 All rights reserved.                  *}
{*********************************************************}
{* poprawki w stosunku do oryginalu S.W. 17.02.92        *}
{*  - zniesiono wlaczanie pliku OPLUS.                   *}
{*  - wstawiono funkcje IsLeapYear i DaysInMonth do ifce *}
{*********************************************************}

unit TpDate;
  {-Date/time routines for use by/with TPENTRY.}
...
function DateStringIsBlank(Picture, S : DateString) : Boolean;
  {-Return True if the month, day, and year in S are all blank or invalid}
...
const
  ...
  begin
    ...
    if not ValidDate(Day, Month, Year) then
      ...
      Dec(Year, MinYear);
      DMYtoDate :=
        {$IFDEF FourByteDates}                        {!!.09}
          ((LongInt(Year div 100)*146097) div 4)+     {!!.09}
          ((LongInt(Year mod 100)*1461) div 4)+       {!!.09}
        {$ELSE}                                       {!!.09}
          ((LongInt(Year)*1461) div 4)+               {!!.09}
        {$ENDIF}                                      {!!.09}
  ...
  function DateStringToDMY(Picture, S : DateString; var D, M, Y : Integer) : Boolean;
    {-Extract day, month, and year from S, returning true if string is valid}
  begin                                         {!!}
    ExtractFromPicture(Picture, S, MonthOnly, M, -1, DefaultMonth); {!!.09}
    ExtractFromPicture(Picture, S, DayOnly,   D, -1, 1);            {!!.09}
    ExtractFromPicture(Picture, S, YearOnly,  Y, -1, DefaultYear);  {!!.09}
  ...
  var
    M, D, Y : Integer;
  begin
    ExtractFromPicture(Picture, S, MonthOnly, M, -1, -1);
    ExtractFromPicture(Picture, S, DayOnly,   D, -1, -1);
    ExtractFromPicture(Picture, S, YearOnly,  Y, -1, -1);
    DateStringIsBlank := (M = -1) and (D = -1) and (Y = -1);
  end;
  ...
  function DMYtoDateString(Picture : DateString; Day, Month, Year : Integer) : DateString;
    {-Merge the month, day, and year into the picture}
  begin
    if Word(Year) < 100 then begin {!!.08}
      Inc(Year, 1900);             {!!.08}
      if Year < Threshold2000 then
        Inc(Year, 100);
    end;
